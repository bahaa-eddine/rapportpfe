...
\begin{figure}[H]
	\center{\includegraphics[width=\textwidth]{Figures/deploiement.PNG}}
	\caption{\label{fig:my-label} Processus de d\'eploiement}
\end{figure}

\begin{enumerate}
\item d\'ep\^ot du code source apr\`es la validation des sections "stages" (test ,build, deploy) qui sont ajouter dans un fichier YAML ".gitlabci.yml".
...
Au cours de la r\'ealisation de la partie frontoffice, on a cr\'ee plus que 170 test unitaire (est une proc\'edure permettant de v\'erifier le bon fonctionnement d'une partie pr\'ecise d'un logiciel ou d'une portion d'un programme) pour valider notre programme et respecter les m\'etriques de sonarqube.
